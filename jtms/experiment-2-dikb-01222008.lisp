...
;; -----------------------------------------------------------------
;; File:      experiment-2-dikb-01222008.lisp
...
(defun data-to-html (out-pth &optional (*JTRE* *JTRE*)
				       (stream *standard-output*))
  "output all ':IN' assertions to a file for processing into an HTML page"
  (setf rslt-str (open out-pth :direction :output :if-exists :supersede))
  (map-dbclass
   #'(lambda (dbclass)
       (setf out "")
...
(defun test-dissertation-DIKB-rule-base-simple-met ()
 ...
 "
  ...
  (in-jtre (create-jtre "Boyce, R.D. UW PhD experiment-2-dikb-01222008 'simple-met' JTRE" :DEBUGGING nil))
  (defvar *JTRE* in-jtre)
  (dolist (form '(       
		  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defun simple-dikb-rule-engine ()
  ...
  (data-to-html "./inference-results")
  )
...
(defun run-experiment-2 ()
  (setf log-path (make-pathname :directory "/tmp" :name "experiment-2-rule-execution.log" ))
  (setf log (open log-path :direction :output :if-exists :append))
  
  (setf prefix "/home/boycer/UW_stuff/DrugInteractions/KnowledgeBase/di-modeling-theory/experiments/loe-experiment-spring-2006/experiment-2-results")
  ;;(setf path (make-pathname :name "paths-to-experiment-2-folders" :directory prefix))  
  ;;(setf path (make-pathname :name "paths-to-experiment-2-folders-POST-16880" :directory prefix))  
  ;;(setf path (make-pathname :name "paths-to-experiment-2-folders-POST-19236" :directory prefix))  
  ;;(setf path (make-pathname :name "paths-to-experiment-2-folders-POST-21582" :directory prefix))  
  ;;(setf path (make-pathname :name "paths-to-experiment-2-folders-POST-23842" :directory prefix))  
  (setf path (make-pathname :name "paths-to-experiment-2-folders-POST-25846" :directory prefix))  
  (setf str (open path :direction :input))
  (setf cntr 25847)
  (setf rd-l (multiple-value-bind (line nl) (read-line str nil nil) (list line nl)))
  (loop 
    (if (eq nil (car rd-l))
	(return))
    (progn 
      (setf line (car rd-l))
      (format log "~%~%~A: SIMPLE-DDI-RULE-ENGINE: Loading the DIKB rule-base" cntr)
      (test-dissertation-DIKB-rule-base-simple-met) ;; resets global *in-jtre*

      (setf asrt-fnm (concatenate 'string  line "/assertions.lisp"))
      (format log "~%   loading assertions generated by the DIKB from file: ~A" asrt-fnm)
      (load asrt-fnm)
      
      (format log "~%   running rules...")
      (run-rules)

      (setf asmpts-fnm (concatenate 'string  line "/changing_assumptions.lisp"))
      (format log "~%   loading assumptions generated by the DIKB from file: ~A" asmpts-fnm)
      (load asmpts-fnm)
      (format log "~%   running rules...")
      (run-rules)

      (format log "~%   setting PKI-1, PKI-2, PKI-3, NO-PKI...")
      (setf PKI-1 (concatenate 'list (get-in-assertions '(1-inhibits-metabolic-clearance-of-2-via-3 ?x ?z ?y)) (get-in-assertions '(1-effects-an-increase-in-2-by-reducing-clearance-of-3-via-4 ?q ?m1 ?x ?enz))))
      (setf PKI-2 (concatenate 'list (get-in-assertions '(1-inhibits-3-the-primary-metabolic-enzyme-of-2 ?y ?z ?enz))))
      (setf PKI-3 (concatenate 'list (get-in-assertions '(1-inhibits-3-the-primary-total-clearance-enz-of-2 ?x ?z ?y)) (get-in-assertions '(1-inhibits-3-the-primary-total-clearance-enz-of-2-and-2-has-high-first-pass ?x ?z ?y))))
      (setf NO-PKI (concatenate 'list (get-in-assertions '(1-does-not-inhibit-the-metabolic-clearance-of-2-via-3 ?x ?z ?y))))

      (setf output-fnm (concatenate 'string  line "/inference-results"))
      (format log "~%   writing data from run to ~A" output-fnm)
      (data-to-html output-fnm)
      
      (format log "~%   resetting data variables and global jtre...")
      (setq PKI-1 nil PKI-2 nil PKI-3 nil NO-PKI nil)
      (defvar *JTRE* nil)
