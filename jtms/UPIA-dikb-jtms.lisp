...
(defun why-group (pattern)
  "apply the why? function to all JTMS nodes that match pattern. For example, try (why-group `(1-INHIBITS-2 ?D ?ENZ))"
  (mapcar #'(lambda (x) (why? x)) (fetch pattern)))
...
(setf drug-class-a-list '((statins . (ATORVASTATIN FLUVASTATIN LOVASTATIN PRAVASTATIN ROSUVASTATIN SIMVASTATIN))
			  (antidepressants . (BUPROPION CITALOPRAM DESIPRAMINE DESVENLAFAXINE DULOXETINE ESCITALOPRAM FLUOXETINE FLUVOXAMINE MIRTAZAPINE NEFAZODONE PAROXETINE R-CITALOPRAM SELEGELINE SERTRALINE TRAZODONE VENLAFAXINE))
			  (sedative-hypnotics . (ESZOPICLONE ZALEPLON ZOLPIDEM))
			  (atypical-antipsych . (ARIPIPRAZOLE ASENAPINE CLOZAPINE ILOPERIDONE LURASIDONE OLANZAPINE PALIPERIDONE QUETIAPINE RISPERIDONE ZIPRASIDONE))
			  (typical-antipsych . (CHLORPROMAZINE FLUPHENAZINE HALOPERIDOL PERPHENAZINE PIMOZIDE THIORIDAZINE THIOTHIXENE))
			  (short-acting-bzd . (ALPRAZOLAM MIDAZOLAM TRIAZOLAM))
			  (antifungal-azoles . (KETOCONAZOLE ITRACONAZOLE FLUCONAZOLE))
			  (antihypertensives . (DILTIAZEM))))

(defun get-PKI-to-tsv (pki-level pki-label ex-txt drug-class)
  "Gather a plain english representation of an and write out in a tab formatted string"
  (setf out "")
  (dolist (a pki-level)
    (let ((object (car (subseq (nth 2 a) 1)))
	  (precip (car (subseq (nth 1 a) 1)))
	  (enzyme (car (subseq (nth 3 a) 1))))

      (if (member object (cdr (assoc drug-class drug-class-a-list)))
	  (setf out (concatenate 'string out (format nil "~%~A	~A	~A	~A	~A" object precip enzyme pki-label  ex-txt)))))) ;
  out)

(defun inhibits-to-tsv (evid-label)
  "Gather a plain english representation of inhibits assertions an and write out in a tab formatted string"
  (setf out "")
  (dolist (a  (fetch '(1-inhibits-2 ?x ?y)))
    (let ((enzyme (car (subseq (nth 2 a) 1)))
	  (precip (car (subseq (nth 1 a) 1))))

      (setf out (concatenate 'string out (format nil "~%~A inhibits ~A	~A" precip enzyme evid-label))))) ;
  out)

(defun inhibits-to-tsv* (lst evid-label)
  "Gather a plain english representation of inhibits assertions an and write out in a tab formatted string"
  (setf out "")
  (dolist (a  lst)
    (let ((enzyme (car (subseq (nth 2 a) 1)))
      ...
      (setf out (concatenate 'string out (format nil "~%~A	~A	~A" precip enzyme evid-label))))) ;
  out)

(defun does-not-inhibit-to-tsv* (lst evid-label)
  "Gather a plain english representation of does-not-inhibit assertions an and write out in a tab formatted string"
  (setf out "")
  (dolist (a  lst)
    (let ((enzyme (car (subseq (nth 2 a) 1)))
	  (precip (car (subseq (nth 1 a) 1))))
...
(defun substrate-of-to-tsv (evid-label)
  "Gather a plain english representation of inhibits assertions an and write out in a tab formatted string"
  (setf out "")
  (dolist (a  (fetch '(1-is-substrate-of-2 ?x ?y)))
    (let ((enzyme (car (subseq (nth 2 a) 1)))
	  (object (car (subseq (nth 1 a) 1))))

      (setf out (concatenate 'string out (format nil "~%~A is substrate of ~A	~A" object enzyme evid-label)))))
  out)

(defun substrate-of-to-tsv* (lst evid-label)
  "Gather a plain english representation of substrate-of assertions an and write out in a tab formatted string"
  (setf out "")
  (dolist (a  lst)
    (let ((enzyme (car (subseq (nth 2 a) 1)))
	  (object (car (subseq (nth 1 a) 1))))

      (setf out (concatenate 'string out (format nil "~%~A	~A	~A" object enzyme evid-label)))))
  out)

(defun is-not-a-substrate-of-to-tsv* (lst evid-label)
  "Gather a plain english representation of is-not-a-substrate-of assertions an and write out in a tab formatted string"
  (setf out "")
  (dolist (a  lst)
    (let ((enzyme (car (subseq (nth 2 a) 1)))
	  (object (car (subseq (nth 1 a) 1))))
...
;; LEFT OFF: implementing this function; use the example (GET-PKI-TO-TSV PKI-1 "PKI-1" "third-tier" 'atypical-antipsych)

(defun data-to-text (tier &optional (*JTRE* *JTRE*) (stream *standard-output*))
  "output all :IN assertions for tier <x> to a tab-delimited file where tier <x> is a string label representing the evidence level used to export assertions from the DIKB"
  (multiple-value-bind
	(second minute hour date month year day-of-week dst-p tz)
      (get-decoded-time)
    (let ((out "")
	  (path (make-pathname :name (concatenate 'string "dikb-drug-drug-interactions-" 
						  (format nil "~2,'0d~2,'0d~2,'0d-~d~2,'0d~d"
							  hour minute second month date year)
						  ".tsv"))))
      (setf fpntr (open path :direction :output :if-exists :supersede))
      (dolist (class (mapcar #'(lambda (x) (car x)) drug-class-a-list))
	(dolist (tag '("PKI-1" "PKI-2" "PKI-3" "NO-PKI"))
	  (setf out (concatenate 'string out (format nil "~A (level ~A)" class tag) (get-pki-to-tsv (eval (intern tag)) tag tier class) (format nil "~%~%")))))
      (format fpntr "~A" out)
      (close fpntr))
    ))
...
(defun test-dissertation-DIKB-rule-base-simple-met ()
 ...
 "
  ...
  (in-jtre (create-jtre "Richard Boyce, PhD UPIA-DIKB 'Exploring the effect of DDIs involving psychotropics on Falls' JTRE" :DEBUGGING t))
  (defvar *jtre* in-jtre)
  (dolist (form '(       
		  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		  ...
		  ;; rule that manages whether 'assumed-effective-dose-of-1-is-2'
		  ;; assumptions are valid compared with the 'minimum-therapeutic-dose-of-1-is-2' 
		  ;; assertion  
		  (rule 
		   ((:IN (minimum-therapeutic-dose-of-1-is-2 ?x ?y))
		    (:IN (assumed-effective-dose-of-1-is-2 ?x ?z) 
		     :TEST (< (eval ?z) (eval ?y))))
		   (retract! (eval (quotize (list `assumed-effective-dose-of-1-is-2 ?x ?z))) 'dikb-inference-assumption))

		  ;; rule that manages whether 'assumed-effective-dose-of-1-is-2'
		  ;; assumptions are valid compared with the 'maximum-therapeutic-dose-of-1-is-2' 
		  ;; assertion  
		  (rule 
		   ((:IN (maximum-therapeutic-dose-of-1-is-2 ?x ?y))
		    (:OUT (assumed-effective-dose-of-1-is-2 ?x ?z) 
		     :TEST (< (eval ?y) (eval ?z))))
		   (retract! (eval (quotize (list `assumed-effective-dose-of-1-is-2 ?x ?z))) 'dikb-inference-assumption))
		  ...
		  (rule 
		   ((:IN (inhibition-constant-of-1-for-2-is-3 ?x ?y ?k_i))  
		    ...
		    (:IN (maximum-in-vivo-concentration-of-1-is-2-at-dose-3 ?x ?c_max ?dose) 
		   ...
		   (rassert! (1-inhibits-2 ?x ?y) 
		    (nil 
		     ...
		     (maximum-in-vivo-concentration-of-1-is-2-at-dose-3 ?x ?c_max ?dose)
		  ...
		  ;; a rule that creates a default assumption
		  ;; asserting that an active ingredient or metabolite
		  ;; does not permanently deactivate the catalytic
		  ;; function of an enzyme if 1) there is a justified
		  ;; or assumed inhibition constant assertion for the
		  ;; drug and enzyme, 2) there are no assertions (:IN
		  ;; or :OUT) that the drug does or does not
		  ;; permanently deactivate the catalytic function of
		  ;; the enzyme
		  (rule 
		   ((:IN (inhibition-constant-of-1-for-2-is-3 ?x ?y ?k_i)
			 :TEST (and 
				(= 0 (length (fetch `(1-permanently-deactivates-catalytic-function-of-2 ?x ?y)))) 
				(= 0 (length (fetch `(1-does-not-permanently-deactivate-catalytic-function-of-2 ?x ?y)))))))
		   (assume! (eval (quotize (list '1-does-not-permanently-deactivate-catalytic-function-of-2 ?x ?y))) 
			    'dikb-inference-assumption))

		  ;; a rule for that handles the contradiction that
		  ;; occurs when an active ingredient or metabolite
		  ;; both permanently and non-permanently deactivates
		  ;; the catalytic function of an enzyme. The default
		  ;; action is to retract the previously assumed
		  ;; assertion that an active ingredient *does not*
		  ;; permanently deactivate the catalytic function of
		  ;; the enzyme
		  (rule 
		   ...
		   (retract!
		    (eval (quotize (list
				    '1-does-not-permanently-deactivate-catalytic-function-of-2 ?drug1 ?enzyme))) 'dikb-inference-assumption))
