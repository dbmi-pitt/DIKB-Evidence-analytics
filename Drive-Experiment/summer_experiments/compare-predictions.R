...
prfx = "/media/scr25/DATA/DRIVE-Experiment/DIKB-Evidence-analytics/Drive-Experiment/summer_experiments/experiment_2/test.tsv"

# read into table
column_names = c("strategy","num_pairs","DIKB-ddi","DIKB-ddi-freq","DIKB-non-DDI","DIKB-non-DDI-freq","DIKB-unknown","DIKB-unknown-freq","both-unknown","val-set-only-unknown","DIKB-only-unknown","tp","fp","tn","fn","ppv","sens","spec","f1","kappa","num-val-set-DDIs","num-val-set-non-DDIs","num-val-set-unknown","average-precision")
ref_table = read.table(prfx, col.names = column_names)
ref_table$coverage = (ref_table$tp + ref_table$tn) / 137
...
headers = c("mean_pred", "mean_unknowns", "mean_f1", "mean_prec", "mean_recall","mean_coverage", "mean_average_precision")

overall_values = c(mean((ref_table$DIKB.ddi+ref_table$DIKB.non.DDI)), mean(ref_table$num_unknowns), mean(ref_table$f1), mean(ref_table$ppv), mean(ref_table$sens), mean(ref_table$coverage), mean(ref_table$average.precision))
both_values = c(mean((both_iv_table$DIKB.ddi+both_iv_table$DIKB.non.DDI)), mean(both_iv_table$num_unknowns), mean(both_iv_table$f1), mean(both_iv_table$ppv), mean(both_iv_table$sens), mean(both_iv_table$coverage), mean(both_iv_table$average.precision))
iv_subs_values = c(mean((iv_sub_table$DIKB.ddi+iv_sub_table$DIKB.non.DDI)), mean(iv_sub_table$num_unknowns), mean(iv_sub_table$f1), mean(iv_sub_table$ppv), mean(iv_sub_table$sens), mean(iv_sub_table$coverage), mean(iv_sub_table$average.precision))
iv_inh_values = c(mean((iv_inh_table$DIKB.ddi+iv_inh_table$DIKB.non.DDI)), mean(iv_inh_table$num_unknowns), mean(iv_inh_table$f1), mean(iv_inh_table$ppv), mean(iv_inh_table$sens), mean(iv_inh_table$coverage), mean(iv_inh_table$average.precision))
no_iv_values = c(mean((no_iv_table$DIKB.ddi+no_iv_table$DIKB.non.DDI)), mean(no_iv_table$num_unknowns), mean(no_iv_table$f1), mean(no_iv_table$ppv), mean(no_iv_table$sens), mean(no_iv_table$coverage), mean(no_iv_table$average.precision))
